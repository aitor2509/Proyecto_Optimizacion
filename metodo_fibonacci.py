...
def fibonacci_search(func, a, b, tol=0.001):
    L = b - a
    n = 3
    while fibonacci(n)[-1] < L / tol:
        n += 1
    fib_series = fibonacci(n)
    
    k = n - 1
    x1 = a + (fib_series[k-2] / fib_series[k]) * (b - a)
    x2 = a + (fib_series[k-1] / fib_series[k]) * (b - a)
    f1 = func(x1)
    f2 = func(x2)
    ...
    for i in range(1, k):
        if f1 > f2:
            a = x1; x1 = x2; f1 = f2; x2 = a + (fib_series[k-i] / fib_series[k-i+1]) * (b-a); f2 = func(x2)
        else:
            b = x2; x2 = x1; f2 = f1; x1 = a + (fib_series[k-i-2] / fib_series[k-i]) * (b-a); f1 = func(x1)
...
def plot_fibonacci(func, x_min, f_min, iterations, a_init, b_init, titulo):
    ...
    ax.set_title(f"Búsqueda de Fibonacci para '{titulo}'")
